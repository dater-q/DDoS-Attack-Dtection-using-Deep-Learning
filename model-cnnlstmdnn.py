...
def create_cnn_lstm_dnn_model(input_shape, num_classes):
    model = Sequential()
...
#loading dataset
df = pd.read_csv("New folder/processed-dataset/cicdos2019-multi-train-pr.csv")
...
model = create_cnn_lstm_dnn_model(input_shape, num_classes)

opt = optimizers.Adam(learning_rate=0.001)
early_stopping = EarlyStopping(monitor='val_loss', patience=10)

model.compile(loss="categorical_crossentropy", optimizer=opt, metrics=['accuracy'])
model.fit(X_train, y_train, epochs=40, batch_size=1000, callbacks=True , validation_data=(X_test, y_test), shuffle=True, class_weight=None, sample_weight=None, initial_epoch=0)
...
#save model
model.save('model-cnn-lstm-dnn.keras')


df1 = pd.read_csv('New folder/processed-dataset/cicdos2019-multi-test-pr.csv')
